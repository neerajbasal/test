...
public class ImageAdapter extends BaseAdapter {
	...
	public ImageAdapter(Context c, Integer[] first, int offst, int nofdays, ArrayList<EventDetails> eventData, int year, int month, int color) {
		mContext = c;
			...
			if (eventDataCell.size() > 0) {
				if (eventDataCell.get(0).getStartDateStr().equals(setStartDate(mThumbIds[position]))) {
					...
					for (int i = 0; i < eventDataCell.size(); i++) {
						...
						tv.setText(eventDataCell.get(i).getTitle());
						if (eventDataCell.get(i).getAvailability() == 1) {
							tv.setTextColor(Color.parseColor("#347C2C"));
						} else if (eventDataCell.get(i).getAvailability() == 0) {
							tv.setTextColor(Color.parseColor("#800517"));
						}
						tv.setTextSize(9f);
